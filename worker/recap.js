...
//
// So a refusal moves down the list rather than ending the attempt. The report
// gets written by the best model that will take it, and the worst case is a
// slightly plainer report instead of no report — which is the whole of what
// "graceful" means here.
//
 ...
 */
async function askModel(model, key, prompt) {
  const ask = (thinking) =>
...
/** What a 200 actually contained: the report, or why there isn't one. */
...
/**
 * Ask Gemini, walking down the waterfall until one of them writes it.
 *
 ...
 * That second half is deliberate: an empty or untagged answer is a *content*
 * failure, and asking four more models to have a go at it would turn one bad
 * report into five requests against quotas that are the reason this list
 * exists. Content failures have their own fixes (MAX_TOKENS, the report tags)
 * and neither of them is "ask somebody else".
 */
export async function writeRecap(env, facts) {
  ...
  for (const model of modelOrder(env)) {
    let res;
    try {
      res = await askModel(model, key, prompt);
    } catch {
