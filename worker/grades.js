...
function describe(p) {
  ...
  // Just the pick, never the margin behind it (§2.46). The vote the organiser
  // types is allowed to move the mark — that's the whole feature — and it
  // stops there: no "won it by one vote", no "three of five". The payload
  // never carries the counts in the first place, so there is nothing here to
...
EVERY LINE LANDS ON ONE NAMED PLAYER, NEVER ON THE TEAM. Four of a player's five teammates got the exact same result, so it is tempting to write the sentence about the shirt instead of the person — "השחורים סיימו אחרונים", "לפחות סיימתם בלי להיאבד" — addressing all five of them at once, or none of them in particular. Don't. The sentence sits next to one name, and has to read as being about that one person even when the fact behind it is shared by the whole team. Say it to them or about them, by name or in the singular — never the plural "you" and never a comment aimed at the team as a whole. A player with nothing of their own that night — no MVP, no run, no drought, no debut — still gets a personal line built the same way, named and singular — its TONE comes from their own mark, exactly as set out below, not from a default dig about riding the team's coat-tails.

MAKE SENSE FIRST, CLEVER SECOND. A sentence a reader has to puzzle over has already failed, however sharp the idea behind it was. If a metaphor needs two clauses to set up before it pays off, it is too complicated for one sentence — cut it down or drop it for something plainer. Before you answer, read every line back as one thought in ordinary spoken Hebrew; if it does not parse cleanly, rewrite it simpler rather than send it as it is.
...
THE PLAYER'S OWN MARK DECIDES WHICH OF THESE YOU REACH FOR — READ IT, NOT JUST THE RESULT. The mark on their line ("ציון X") already blends the team's night with their MVP pick, career and form, and it is the one number that says how THIS person is actually doing, not just their shirt. A high mark, roughly 7 and up, means treat them as someone playing well: reach for the coronation, the hype, the wild exaggeration of how good they were — a genuine compliment wearing a joke — even when the only fact on their line is the team's shared result. It is NOT the moment for a dig about luck, freeloading, riding the team's coat-tails or not deserving it — that undercuts a mark that says the opposite. Save the sharpest, most deflating jokes — the proper ribbing — for a genuinely low mark, or a player whose team got hammered or has not won in a month. A mark in the middle gets one of these moves played completely deadpan — dry and flat is a joke; a shrug in a nicer font is not.
