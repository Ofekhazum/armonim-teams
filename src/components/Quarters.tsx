...
//
// The old version of this was four bars labelled `32/49`, `23/44`, `19/43`,
// `29/42`, under the heading "where their wins land". Every number on it was
// true and it was still unreadable, for three separate reasons worth writing
...
//      fade, or arrive late.
//
// So: the rate is stated as a percentage in words, the raw count is kept
// underneath as the evidence for it, and a dashed line across the plot marks
// **their own average across the whole night**. That last one is what turns
// four numbers into a shape — every bar is now read against the same line, and
// the question "is 65% good" is replaced by "is this quarter better than their
// other three", which the data can actually answer.

const LABELS = ['1st', '2nd', '3rd', '4th'];

// Bars are drawn against a full-height track, so a 100% quarter fills it. The
// alternative — scaling to the player's best quarter — makes every profile use
// a different axis and turns four flat bars into a dramatic-looking staircase.
const pct = (r: number) => `${Math.round(r * 100)}%`;

export default function Quarters({ arcs }: { arcs: Arcs }) {
  ...
  return (
    <div>
      ...
      <div className="relative h-24 overflow-hidden rounded-xl bg-amber-900/[0.05]">
        <div className="absolute inset-0 flex items-end gap-1.5 px-1.5">
          {arcs.quarters.map((q, i) => {
            ...
            return (
              <div
                key={i}
                title={`${q.won} of ${q.played} matches won in the ${LABELS[i]} quarter`}
                ...
                // A quarter nobody played gets no bar at all rather than a
        ...
        {/* Their own average, across all four. Positioned in the same
          ...
          return (
            <div key={i} className="flex-1 text-center">
              ...
              <div className="font-mono text-[10px] leading-tight text-amber-900/35">
                {q.played ? `${q.won} of ${q.played}` : 'none played'}
              </div>
            </div>
          );
        })}
